...
2.1 Provide filtering for movies by genres, actors and title. Use `?actors=`, `?genres=` and `?title=` parameters.
Filtering by title with the `string` parameter should return all movies whose title contains `string`.

3.1 Implement filtering for movie sessions by date and movie. The date should be provided in `year-month-day` format,
the movie by its id.
Example:
...
4.1 Return taken places for movie session details endpoint
