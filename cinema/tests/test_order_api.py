...
class OrderApiTests(TestCase):
    def setUp(self):
        ...
        self.actress = Actor.objects.create(first_name="Kate",
                                            last_name="Winslet")
        ...
        self.movie_session = MovieSession.objects.create(
            movie=self.movie, cinema_hall=self.cinema_hall,
    ...
    def test_movie_session_detail_tickets(self):
        ...
        self.assertEqual(response.data["taken_places"][0]["row"],
                         self.ticket.row)
        self.assertEqual(response.data["taken_places"][0]["seat"],
                         self.ticket.seat)

    def test_movie_session_list_tickets_available(self):
        response = self.client.get(f"/api/cinema/movie_sessions/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data[0]["tickets_available"],
