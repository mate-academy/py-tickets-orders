...
class MovieSessionViewSet(viewsets.ModelViewSet):
    ...
    @staticmethod
    def _date_str_to_list(date):
        return [int(value) for value in date.split("-")]
    ...
    def get_queryset(self):
        ...
        if date:
            ...
            queryset = queryset.filter(
                show_time__date=datetime.date(date_list[0], date_list[1], date_list[2])
            )
...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.action == "list":
            queryset.prefetch_related("tickets__movie_session__cinema_hall")
