...
class MovieViewSet(viewsets.ModelViewSet):
    ...
    @staticmethod
    def search_int(element):
        return [int(str_id) for str_id in element.split(",")]
    ...
    def get_queryset(self):
        ...
        if actors:
            actors_id = self.search_int(actors)
            queryset = queryset.filter(actors__id__in=actors_id)

        if genres:
            genres_id = self.search_int(genres)
...
class MovieSessionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if movie:
            movie_id = self.search_int(movie)
