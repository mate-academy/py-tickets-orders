...
class TicketSerializer(serializers.ModelSerializer):
    def validate(self, attrs):
        seats_in_row = attrs["movie_session"].cinema_hall.seats_in_row
        rows = attrs["movie_session"].cinema_hall.rows

        if not (1 <= attrs["row"] <= rows):
            raise serializers.ValidationError(
                f"row must be in range (1, "
                f"{attrs['movie_session'].cinema_hall.rows}),"
                f"not {attrs['row']}"
            )
        elif not (
                1 <= attrs["seat"] <= seats_in_row
        ):
            raise serializers.ValidationError(
                f"seat must be in range (1, "
                f"{attrs['movie_session'].cinema_hall.seats_in_row}),"
                f"not {attrs['seat']}"
            )
